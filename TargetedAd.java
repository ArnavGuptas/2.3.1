...
public class TargetedAd {
  ...
  public static void main(String[] args)
  {
    ...
    d.setData("socialMediaPostsSmall.txt", "targetWords.txt");
    
    String usernames = "";
    ArrayList<String> targetedPosts = d.getPosts();
    
    targetedPosts.removeIf((String post) -> {return d.containsTargetWord(post);});
    ArrayList<String> posts = d.getPosts();
    
    for (int i = 0; i < targetedPosts.size(); i++) {
        String username = targetedPosts.get(i).split(" ")[0];
        usernames = usernames + " " + username;
    }
    d.prepareAdvertisement("test.txt", usernames, "Buy our horse feed!");
  }
