...
public class TargetedAd {
  ...
  public static void main(String[] args)
  {
    ...
    String nonTargetedUsernames = "";

    ArrayList<String> targetedPosts = new ArrayList<>(d.getPosts());
    targetedPosts.removeIf((String post) -> {return d.containsTargetWord(post);});
    ArrayList<String> nonTargetedPosts = new ArrayList<>(d.getPosts());
    nonTargetedPosts.removeAll(targetedPosts);
      
    for (int i = 0; i < nonTargetedPosts.size(); i++) {
        String nonTargetedUsername = nonTargetedPosts.get(i).split(" ")[0];
        if (!nonTargetedUsernames.contains(nonTargetedUsername))
          nonTargetedUsernames = nonTargetedUsernames + " " + nonTargetedUsername;
    }

    for (int i = 0; i < targetedPosts.size(); i++) {
        String targetedUsername = targetedPosts.get(i).split(" ")[0];
        if (!targetedUsernames.contains(targetedUsername))
          targetedUsernames = targetedUsernames + " " + targetedUsername;
    }
    
    d.prepareAdvertisement("target.txt", targetedUsernames, "Buy our horse feed!");
    d.prepareAdvertisement("nottarget.txt", nonTargetedUsernames, "I heard you like animals! You should buy a horse! We'll sell you horse food for cheap!");
